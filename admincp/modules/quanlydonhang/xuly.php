...
elseif (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['code'])) {
    ...
    $sql_cart_details = "SELECT id_sanpham, soluongmua FROM tbl_cart_details WHERE code_cart='" . $code_cart . "'";
...
elseif (isset($_GET['action']) && $_GET['action'] == 'delete_product' && isset($_GET['id_cart_details']) && isset($_GET['id_sanpham']) && isset($_GET['soluongmua'])) {
    $id_cart_details = $_GET['id_cart_details'];
    ...
    $sql_delete_cart_detail = "DELETE FROM tbl_cart_details WHERE id_cart_details = $id_cart_details";
    mysqli_query($mysqli, $sql_delete_cart_detail);

    // Kiểm tra xem còn sản phẩm nào trong đơn hàng không
    $sql_check_cart = "SELECT COUNT(*) as count FROM tbl_cart_details WHERE code_cart='" . $code_cart . "'";
