...
<script src="../../js/suadonhang.js"></script>
...
if (isset($_POST['update_quantity'])) {
    ...
    foreach ($soluongmua as $id_cart_details => $soluong) {
        ...
        if ($quantity_change < 0 && - ($quantity_change) > $current_stock) {
            ...
            // Cập nhật số lượng sản phẩm trong bảng tbl_cart_details
            $sql_update_quantity = "UPDATE tbl_cart_details SET soluongmua='$soluong' WHERE id_cart_details='$id_cart_details'";
            mysqli_query($mysqli, $sql_update_quantity);
    ...
    if (!empty($errors)) {
        ...
        $sql_cart = "SELECT SUM(tbl_sanpham.giasp * tbl_cart_details.soluongmua) AS tongtien 
...
// Để sau Lấy số lượng hàng đã được đặt từ đơn hàng
