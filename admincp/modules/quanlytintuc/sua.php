<?php
$sql_sua_bv = "SELECT * FROM tbl_tintuc WHERE id='$_GET[idbaiviet]'LIMIT 1";
$query_sua_bv = mysqli_query($mysqli, $sql_sua_bv);
?>

<p>
<h1 class="title-sua">SỬA TIN TỨC</h1>
</p>

<table class="bang-sua" border="1" width="100%" border-collapse=collapse>
    <?php
    while ($row = mysqli_fetch_array($query_sua_bv)) { ?>
        <form method="POST" action="modules/quanlytintuc/xuly.php?idbaiviet=<?php echo $_GET['idbaiviet'] ?>"  enctype="multipart/form-data">
            <tr>
                <td>Tên tin tức</td>
            ...
            <tr>
                <td>Tóm tắt</td>
                <td><textarea rows="10" name="tomtat" id="tomtattintuc" style="resize:none"><?php echo $row['tomtat'] ?></textarea></td>
            </tr>

            <tr>
                <td>Nội dung</td>
                <td><textarea rows="10" name="noidung" id="noidungtintuc" style="resize:none"><?php echo $row['noidung'] ?></textarea></td>
            </tr>
            ...
            <tr>
                <td colspan="2"><input type="submit" name="suabaiviet" value="Sửa tin tức"></td>
            </tr>
        </form>
    <?php
    }
